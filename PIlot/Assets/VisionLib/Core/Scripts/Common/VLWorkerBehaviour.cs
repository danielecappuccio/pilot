...
{
    [Tooltip("Name of the licence file, e.g. licence.xml")]
    public VLLicenseFile licenseFile;

    public string calibrationDataBaseURI;

    public VLUnityCameraHelper.FlipCoordinateSystemHandedness flipCoordinateSystemHandedness = 
    ...
    /// </remarks>
    [Tooltip("Directory from which the tracking configuration files will be loaded. If empty: StreamingAssets/VisionLib is used")]
    ...
    public void StartTracking(string filename)
    {
        ...
        if (filename.StartsWith("http://") ||
            ...
            filename.StartsWith("local_storage_dir:") ||
    ...
    public void ResetTracking()
    {
        ...
        this.worker.PushCommand(
            new VLWorkerCommands.ResetTrackingCmd(),
    ...
    public void ResetTrackingSoft()
    {
        ...
        this.worker.PushCommand(
            new VLWorkerCommands.ResetSoftCmd(),
    ...
    /// You can call this function in order to get useful system information before starting the tracking pipe
    ...
    public bool GetTrackerType(out string trackerType)
    {
        if (this.aap != null)
        {
            return this.aap.GetTrackerType(out trackerType);
        }
        trackerType = "";
        return false;
    }
...
// Set the path to the license file
    ...
    /* Camera calibration specific function which are going to deprecate in favor of a new behaviour */
...
#if UNITY_EDITOR
[CustomEditor(typeof(VLWorkerBehaviour))]
public class VLWorkerBehaviourEditor : Editor
{
    ...
    {
        if (this.serializedObject.FindProperty("flipCoordinateSystemHandedness").enumValueIndex ==
            (int) VLUnityCameraHelper.FlipCoordinateSystemHandedness.Manually)
        {
            EditorGUILayout.Space();
            EditorGUILayout.HelpBox(
                "Manually flipping the handedness of the coordinate systems is deprecated. " +
                    "\nPlease remove the 180 degree rotation from your model and use the Automatically option.",
                MessageType.Warning);
        }
    }
}
