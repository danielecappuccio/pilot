...
public class VLDebugImageBehaviour : VLWorkerReferenceBehaviour
{
    ...
    private void UpdateImageSize()
    {
        ...
        if (this.workerBehaviour)
        {
            this.workerBehaviour.GetTrackerType(out trackerType);
        }

        // don't rotate image on HoloLens
        if (trackerType != "HololensModelTracker")
        {

            if (this.renderRotation == VLRenderRotation.CCW0)
            {
                this.imageRectTransform.rotation = rot0;
            }
            else if (this.renderRotation == VLRenderRotation.CCW180)
            {
                this.imageRectTransform.rotation = rot180;
            }
            else if (this.renderRotation == VLRenderRotation.CCW90)
            {
                this.imageRectTransform.rotation = rot90;
            }
            else if (this.renderRotation == VLRenderRotation.CCW270)
            {
